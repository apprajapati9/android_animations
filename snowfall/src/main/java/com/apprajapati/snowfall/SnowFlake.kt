 ...
 */
internal class SnowFlake(private val randomizer: Randomizer,
                         val params: Params
) {
    ...
    // get paint object, if null initialize.
    private val paint: Paint get() {
        if (_paint == null){
    ...
    internal fun reset(positionY: Double? = null) {
        ...
        size = randomizer.randomInt(params.sizeMinInPx,
                                    params.sizeMaxInPx,
                                    gaussian = true)
        ...
        val speed = ((size - params.sizeMinInPx).toFloat() /
                (params.sizeMaxInPx - params.sizeMinInPx)
                * (params.speedMax - params.speedMin) + params.speedMin)

        val angle = toRadians(randomizer.randomDouble(params.angleMax) *
                                    randomizer.randomSignum())
    ...
    fun isStillFalling() : Boolean {
        return (shouldRecycleFalling || (positionY > 0 &&
                positionY < params.parentHeight))
    }
    ...
    fun update(){
        ...
        if(params.fadingEnabled){
            paint.alpha = (alpha * ((params.parentHeight - positionY).toFloat()
                        / params.parentHeight)).toInt()
    ...
    fun draw(canvas: Canvas){
        ...
        if(localBitmap != null){
            canvas.drawBitmap(localBitmap, positionX.toFloat(),
                            positionY.toFloat(), paint)
        }
        else {
